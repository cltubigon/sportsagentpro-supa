...
const useUpdateIndividualPostApplicantsMutateData = (query) => {
  const { mainKey, pageNumber, eqValue } = query
  ...
  return useMutation((updateValue) => updateMutateData(query, updateValue), {
    onMutate: async (updateValue) => {
      ...
      queryClient.setQueryData(mainKey, (oldQueryData) => {
        const newData = [{ ...oldQueryData[0], postApplicants: updateValue }]
        return newData
      })
      queryClient.setQueryData(["posts", pageNumber], (oldQueryData) => {
...
export default useUpdateIndividualPostApplicantsMutateData
